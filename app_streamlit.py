...
if mode == "Manuale":
    ...
    if BINANCE_API_KEY and BINANCE_API_SECRET:
        try:
            ...
            st.error(f"Errore durante la connessione a Binance: {str(e)}")
    else:
...
if question and OPENAI_API_KEY:
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"Errore API OpenAI: {response.status_code} - {response.text}")
    except Exception as e:
        st.error(f"Errore durante la chiamata all'AI: {str(e)}")
...
for symbol, qty in st.session_state.wallet.items():
    try:
        ...
        st.warning(f"Errore nel recupero del prezzo per {symbol}: {str(e)}")
        data.append({"symbol": symbol, "quantity": qty, "usd_value": "N/D"})

try:
    df = pd.DataFrame(data)
    st.dataframe(df)
except Exception as e:
    st.error(f"Errore nella visualizzazione del wallet: {str(e)}")
...
try:
    ...
    # Calcolo RSI con gestione errori
    try:
        ...
        # Fallback: mostra solo il grafico del prezzo se il RSI fallisce
        try:
            st.warning(f"Errore nel calcolo RSI: {str(e)}. Mostro solo il grafico del prezzo.")
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=df_prices["timestamp"], y=df_prices["price"], name="Prezzo"))
            fig.update_layout(
                title=f"Prezzo di {asset.capitalize()}",
                xaxis_title="Data",
                yaxis_title="Prezzo (USD)"
            )
            st.plotly_chart(fig, use_container_width=True)
        except:
            st.error("Errore nella generazione del grafico base")

except Exception as e:
    st.error(f"Errore nel recupero dei dati di mercato per {asset}: {str(e)}")